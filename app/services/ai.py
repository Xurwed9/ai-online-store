...
АБСОЛЮТНО ЗАПРЕЩЕНО:
- Показывать пользователю любой код, JSON, вызовы инструментов или технические детали.
- Писать блоки ```tool_call``` в ответе пользователю. Они только для внутреннего вызова.
- Говорить пользователю про "инструменты", "API", "функции", "tool_call".

ПРАВИЛА:
1. Если пользователь описывает товар — ищи его через search_products. Покажи результат красиво: название, цена, размер, цвет, наличие.
2. Если товар не найден — предложи альтернативы через recommend_products или search_products с другими параметрами. Не просто скажи "нет".
...
7. Если пользователь спрашивает "что мне купить" или "что посоветуешь" — используй recommend_products для персонализированных рекомендаций.
8. Если пользователь смотрит товар — предложи похожие через get_similar_products и покажи "часто покупают вместе" через get_frequently_bought_together.
9. Если товар закончился на складе — предложи альтернативы из той же категории.
10. Учитывай предпочтения пользователя (если они указаны в профиле) при рекомендациях.
11. Никогда не показывай пользователю результаты вызовов инструментов в сыром виде. Форматируй ответ красиво и понятно.
12. Если пользователь хочет изменить количество — используй update_cart_quantity.
13. Если пользователь хочет удалить товар из корзины — используй remove_from_cart.
14. Если пользователь спрашивает итого или "сколько стоит" — используй get_cart_total для расчёта со скидками.

ДОСТУПНЫЕ ИНСТРУМЕНТЫ (вызывай их когда нужно, НЕ показывай пользователю):
{tools}

ФОРМАТ ОТВЕТА:
- Отвечай ТОЛЬКО текстом, понятным пользователю.
- Если нашёл товары — покажи их в красивом формате: "🔹 Название — $Цена | Размер: X | Цвет: Y | В наличии: Z шт."
- Если нужен вызов инструмента — используй JSON-блок (ОН НЕ ВИДЕН ПОЛЬЗОВАТЕЛЮ, только система его обработает):
```tool_call
...
def _strip_tool_calls(text: str) -> str:
    """Удалить все tool_call блоки из текста, чтобы пользователь не видел JSON."""
    return re.sub(TOOL_CALL_PATTERN, "", text).strip()
    ...
    if match:
        try:
            ...
            try:
                ...
                final_text = _strip_tool_calls(followup_response.choices[0].message.content)
            except Exception as e:
                logger.exception("Error in AI followup after tool call")
                final_text = "Извините, произошла ошибка при обработке ответа. Попробуйте ещё раз."
            ...
            return {
                "reply": _strip_tool_calls(assistant_text),
                "tool_called": None,
                "tool_result": None,
                "error": str(e),
            }
